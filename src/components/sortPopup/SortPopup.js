import React, { useState, useRef, useEffect } from "react";


const SortPopup = React.memo( function SortPopup ({items}) {
  const [visiblePopup, setVisiblePopup] = useState(false);
  const [activeSortItem, setActiveSortItem] = useState(0);
  const sortRef = useRef();
  const activeLabel = items[activeSortItem].name;

  const togglePopup = () => { 
    setVisiblePopup(!visiblePopup);
  };

  const handleOutsideClick = (event) => { 
    if (!event.path.includes(sortRef.current)) { 
      setVisiblePopup(false)
    }
  };       
  
 const onSelectSortItem = (index) => { 
   setActiveSortItem(index);
   setVisiblePopup(false);
 }


 useEffect(() => {
  document.body.addEventListener('click', handleOutsideClick);
 }, [])

  const sortPopup = items?.map((obj, index) => ( 
    <li className={activeSortItem === index ? 'active' : ''}
        key={`${obj.type}_${index}`}
        onClick = {() => onSelectSortItem(index)}>
        {obj.name}
    </li>
));

return (
  <div>
    <div className="sort" ref={sortRef}>
      <div className="sort__label">
        <svg style={visiblePopup ? {transform: "rotate(180deg)"} : null}
          width="10"
          height="6"
          viewBox="0 0 10 6"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z"
            fill="#2C2C2C"
          />
        </svg>
        <b>Сортировка по:</b>
        <span onClick = {togglePopup}>{activeLabel}</span>
      </div>
      {visiblePopup &&  
      <div className="sort__popup">
        <ul>
          {sortPopup}
        </ul>
        </div>}
    </div>
  </div>
);
}
)
export default SortPopup



// const [visiblePopup, setVisiblePopup] = useState(false);
// const [activeSortItem, setActiveSortItem] = useState("");
// const [selectedItem, setSelectedItem] = useState("популярности");


// const sortPopup = props.categories.map((item, index) => ( 
//     <li className={activeSortItem === index ? 'active' : ''}
//         key={index}
//         onClick = {() => handleSortItem(item,index)}>
//         {item}
//     </li>
// ));

// const handleSortItem = (item, index) => { 
//     setActiveSortItem(index);
//     setVisiblePopup(false);
//     setSelectedItem(item);
// }       

// const togglePopup = () => { 
//   setVisiblePopup(!visiblePopup);
// }
